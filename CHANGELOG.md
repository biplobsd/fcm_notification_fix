...
## Unreleased
### Fixed
- **AOT Cache Survival — ART's Nightly Cleanup**: `BackgroundDexoptJobService` (periodic, charging + idle) ends with an ART Service cleanup pass that unlinks every `/data/dalvik-cache` artifact ART did not produce — every odex `compile_aot_cache` writes. The running `system_server` keeps the mapped inodes, so nothing is felt until the next reboot, which then starts with no odex and drops to JIT: the regime `--boot-image` (v1.4 → #24) was meant to end. Measured on `goku` / `OS3.0.309.0.WNVCNXM`: artifacts gone between 02:25 and 02:31, two nights, two builds. `compile_aot_cache` now also archives what it publishes under `$MODDIR/cache/<isa>/` (a hard link — same inode, no copy; a copy when the module directory is on another filesystem) with a manifest, and `post-fs-data.sh` puts any missing name back before zygote. Nothing outlives the module: no mounts, no immutable flags; the archive is a plain directory removed with it, and the dalvik-cache names stay plain files that any wipe, cleanup or recovery removes as before. `uninstall.sh` and the install-failure path remove the archive and, by manifest, its dalvik-cache names. An archive that could not be written completely is reported at install and re-patch time (`AOT_ARCHIVE_WARNING`) rather than failing the compile: a cache that works until the first cleanup beats a wiped one. The archive is replaced atomically — staged next to the old one, manifest written last, then swapped by rename — with `fsync` on the staged files and directories where toybox provides it — so a reboot or power loss mid-install leaves either the previous archive or the new one intact, and `restore_aot_cache` recovers the one case a crash between the two renames can leave behind, never discarding `.old` until a complete archive is in place.
...
### Tests
- `tests/aot_cache_survival_test.sh`: 54 checks — `compile_aot_cache` against a stub `dex2oat` archives exactly what it published; a simulated cleanup is undone with the same inodes; restore is idempotent and never overwrites a present name; the cross-filesystem copy fallback round-trips; an archive that cannot be written leaves the compile standing and raises the warning; the atomic swap leaves nothing behind and every interruption state (stale staging dir, `.old` only, leftover `.old`) still restores; `forget_aot_cache` clears both places. Wired into `run_ci_tests.sh` as step 0.
