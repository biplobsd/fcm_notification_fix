...
### Tests
- `tests/aot_cache_survival_test.sh`: 33 checks — `compile_aot_cache` against a stub `dex2oat` archives exactly what it published; a simulated cleanup is undone with the same inodes; restore is idempotent and never overwrites a present name; the cross-filesystem copy fallback round-trips; an archive that cannot be written leaves the compile standing and raises the warning; `forget_aot_cache` clears both places. Wired into `run_ci_tests.sh` as step 0.
